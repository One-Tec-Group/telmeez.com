...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        // $credential = array('email' => $email, 'password' => sha1($password), 'status' => 0);
    ...
    public function register() {
        ...
        if ($validity) {
            ...
            if ($data['role_id'] == 'student') {
              $data['role_id']  = 3;
            } elseif ($data['role_id'] == 'instructor') {
              $data['role_id']  = 2;
              $this->session->set_userdata('instractor_form', $data);
              redirect(site_url('home/instructorForm'));
            }
    ...
    public function register_instractor() {
      $data = $this->session->userdata('instractor_form');
      $data['specialized_fields'] = html_escape($this->input->post('specialized_fields'));
      $data['teach_experience']  = html_escape($this->input->post('teach_experience'));
      $data['expertise']  = html_escape($this->input->post('expertise'));
